...
from apartment_agent.main import build_runner, main
from apartment_agent.models import AppConfig, DemoAdapterConfig, ImmoweltAdapterConfig, TelegramConfig, EmailConfig
...
def base_config() -> AppConfig:
    return AppConfig(
        ...
        regions=["Boizenburg"],
        language="en",
        database_path="apartments.sqlite3",
...
def test_build_runner_returns_runner_with_demo_adapter():
    with patch("apartment_agent.main.load_config", return_value=base_config()):
...
def test_build_runner_raises_when_no_adapters_enabled():
    ...
    with patch("apartment_agent.main.load_config", return_value=config):
...
def test_main_runs_once_mode_calls_run_once():
    ...
    with patch("apartment_agent.main.build_runner", return_value=fake_runner):
        with patch("sys.argv", ["apartment-agent", "--once"]):
            main()
...
def test_main_scheduler_mode_starts_scheduler():
    ...
    with patch("apartment_agent.main.build_runner", return_value=fake_runner):
        with patch("apartment_agent.main.BlockingScheduler", return_value=scheduler_instance):
            with patch("sys.argv", ["apartment-agent"]):
                main()
...
def test_main_report_open_uses_management_mode_without_runner():
    ...
    with patch("apartment_agent.main.build_runner") as build_runner_mock:
        with patch("apartment_agent.main.load_config", return_value=base_config()):
            with patch("apartment_agent.main.ApartmentDatabase", return_value=_database_context(database_mock)):
                with patch("apartment_agent.main.generate_open_apartments_markdown", return_value="# report"):
                    with patch("builtins.print") as print_mock:
                        with patch("sys.argv", ["apartment-agent", "--report-open"]):
                            main()
    ...
    with patch("apartment_agent.main.load_config", return_value=base_config()):
        with patch("apartment_agent.main.ApartmentDatabase", return_value=_database_context(database_mock)):
            with patch("builtins.print"):
                with patch("sys.argv", ["apartment-agent", "--delete-key", "immowelt:123"]):
                    main()
...
def test_main_delete_older_than_days_calls_database_cleanup():
    ...
    with patch("apartment_agent.main.load_config", return_value=base_config()):
        with patch("apartment_agent.main.ApartmentDatabase", return_value=_database_context(database_mock)):
            with patch("builtins.print"):
                with patch("sys.argv", ["apartment-agent", "--delete-older-than-days", "7"]):
                    main()
...
def test_main_report_output_writes_file(tmp_path):
    ...
    with patch("apartment_agent.main.load_config", return_value=base_config()):
        with patch("apartment_agent.main.ApartmentDatabase", return_value=_database_context(database_mock)):
            with patch("apartment_agent.main.generate_open_apartments_markdown", return_value="# markdown report"):
                with patch("builtins.print"):
                    with patch(
                        "sys.argv",
                        [
